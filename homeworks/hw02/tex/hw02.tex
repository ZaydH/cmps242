...
\begin{document}
  ...
  \begin{subproblem}{a}
    If a box is chosen at random with probabilities, $p(r)=0.2$, $p(b)=0.2$, and~$p(g)=0.6$, and a piece of fruit is removed from the box (with equal probability of select any of the items in the box), then what is the probability of selecting an apple?
  \end{subproblem}
  ...
  \begin{subproblem}{b}
    Show the maximum of the Gaussian distribution is given by~$\bm{\mu}$.
  \end{subproblem}
  ...
  \begin{aligncustom}
    \ln \left\{ \mathcal{N}(\mathbf{x} | \bm{\mu}, \Sigma) \right\}  &= \ln \left( \frac{1}{(2\pi)^{\frac{n}{2}} | \Sigma | ^ {1/2} } \exp\left\{-\frac{1}{2} \left(\mathbf{x}-\bm{\mu} \right)^{\textrm{T}}\Sigma^{-1}\left(\mathbf{x}-\bm{\mu}\right) \right\} \right) \\~\\

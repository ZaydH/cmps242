...
\begin{document}
  ...
  \begin{aligncustom}
    \ln \mathcal{N}(\mathbf{x} | \bm{\mu}, \Sigma)  &= \ln \left( \frac{1}{(2\pi)^{\frac{n}{2}} | \Sigma | ^ {1/2} } \exp\left\{-\frac{1}{2} \left(\mathbf{x}-\bm{\mu} \right)^{\textrm{T}}\Sigma^{-1}\left(\mathbf{x}-\bm{\mu}\right) \right\} \right) \\~\\
    &= \ln \left( \frac{1}{(2\pi)^{\frac{n}{2}}} \right) + \ln \left( \exp\left\{-\frac{1}{2} \left(\mathbf{x}-\bm{\mu} \right)^{\textrm{T}}\Sigma^{-1}\left(\mathbf{x}-\bm{\mu}\right) \right\} \right) \\~\\
  ...
  \begin{aligncustom}
    \int {p(x) \ln q(x)} dx &= \int {p(x) \ln \left( \frac{1}{\sqrt{2\pi s^2}}e^{\frac{-(x-m)^2}{2s^2}} \right) } dx \textrm{.}
  \end{aligncustom}

  This can be reduced via:
  
  \begin{aligncustom}
    \int {p(x) \ln q(x)} dx  &= \int {p(x) \left( \ln \left( \frac{1}{\sqrt{2\pi s^2}} \right)  + \ln \left(e^{\frac{-(x-m)^2}{2s^2}} \right) \right) } dx \\~\\
    &= -\frac{1}{2} \ln \left( 2\pi s^2 \right) \int {p(x)} dx + \int {p(x) \frac{-(x-m)^2}{2s^2} } dx \\~\\
    &= -\frac{1}{2} \ln \left( 2\pi s^2 \right) + \int {p(x) \frac{-(x-m)^2}{2s^2} } dx 
  \end{aligncustom}
...
ting for~$p(x)$ and~$q(x)$ changes the natural logarithm in the previous function to:
  ...
  \begin{aligncustom}
    ...
    &= \ln \left(  \frac{\sigma}{s} \right) + \frac{-(x-m)^2}{2s^2}+\frac{(x-\mu)^2}{2 \sigma^2} \textrm{.}
  \end{aligncustom}
  ...
  \begin{equation}
    f(\lambda x_{1} + (1-\lambda) x_{2}) \leq \lambda f(x_{1}) + (1-\lambda) f(x_{2}) \textrm{.}
  \end{equation}

  Define a second function,~$g(x)$, to be convex and a third function,~$h(x)=f(x)+g(x)$.  Add the definition of convexity for~$f$ to the definition of convexity for~$g$ as in Eq.~\eqref{eq:convexEquation}.
  ...
  This can be rearranged due to the associativity and commutativity of addition.
